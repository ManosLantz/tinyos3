...
void boost(){
	...
	for (int i = 0; i < priority_queues-2; i++)
	{
		while(!is_rlist_empty(&SCHED[i]))
		{
			rlnode* curr =rlist_pop_front(&SCHED[i]);
			curr->tcb->priority++;
			rlist_push_back(&SCHED[i+1],curr);
		}
	}
	return ;
}
...
void yield(enum SCHED_CAUSE cause)
{
	...
	boostCounter++;
	if(boostCounter==boostCounterMax-1)
	{
		//boost();
		boostCounter=0;
	}	
