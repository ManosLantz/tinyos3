...
static inline void rlfreeList(rlnode* head)///source:https://stackoverflow.com/questions/6417158/c-how-to-free-nodes-in-the-linked-list
{
   rlnode* tmp1 =head;
   rlnode* tmp =head;
   while (is_rlist_empty(tmp)!=0)
    {
       tmp1 =rlist_pop_front(tmp);
       if(tmp1->next!=NULL){
       	tmp=tmp1->next;
       } else{
       	break;
